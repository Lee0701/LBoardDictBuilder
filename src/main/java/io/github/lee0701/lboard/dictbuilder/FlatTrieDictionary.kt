...
class FlatTrieDictionary(data: ByteArray): Dictionary {
    ...
    override fun search(word: String): List<Word> {
        return listWords(searchAddress(word) ?: return listOf(), word)
    }

    override fun searchPrefix(prefix: String, length: Int): List<Word> {
        return searchRecursive(searchAddress(prefix) ?: return listOf(), prefix, length)
    }

    override fun searchSequence(seq: List<Int>, layout: Map<Int, List<Int>>): List<Word> {
    ...
    private fun searchRecursive(address: Int, current: String, length: Int): List<Word> {
    ...
    private fun searchSequenceRecursive(seq: List<Int>, layout: Map<Int, List<Int>>, address: Int, current: String, index: Int): List<Word> {
    ...
    private fun listWords(address: Int, word: String): List<Word> {
        ...
        for(i in 0 until wordSize) {
            ...
            p += 4
            result += Word(word, pos.toInt(), frequency)
        }
        return result
    }
